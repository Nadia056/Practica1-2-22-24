...
<body>
    <div class="container-fluid">
        <div class="d-flex mt-4 ">
            <a href="{{route('CoordHome',['id'=>$id])}}" id="a" class="btn btn-primary mb-2"><i
        ...
        <div class="d-flex justify-content-end">
            <button type="button" id="a" class="btn btn-primary mb-2" data-bs-toggle="modal"
                data-bs-target="#createModal">Create new product</button>
        </div>
        @if($products->isEmpty())
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <button type="button" class="btn btn-primary  btn-sm edit-btn" onclick="openEditModal(this)"
                                data-productid="{{$product->id}}" data-productname="{{$product->name}}"
                                data-productdesc="{{$product->description}}" data-productprice="{{$product->price}}"
                                data-categoryname="{{$product->category}}">Update</button>

                            <button type="button" class="btn btn-danger btn-sm edit-btn"
                                data-productid="{{$product->id}}" onclick="openDeleteModal(this)" data->Delete</button>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action=""></form>
                    <form action="{{route('Coord.createProduct',['id'=>$id])}}" method="POST">
                        @csrf
                        <div class="mb-3">
    ...
    <div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{route('Coord.updateProduct',['id'=>$id])}}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            <label for="name" class="form-label">Name</label>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form action="{{route('Coord.deleteProduct',['id'=>$id])}}" method="POST">
                        @csrf
                        @method('PUT')
    ...
    for (var i = 0; i < categorySelect.options.length; i++) { // Si el valor de la opción coincide con el nombre de la categoría del producto, marcarla como seleccionada 
        ...
        categorySelect.selectedIndex=i; break; } } var modal=new bootstrap.Modal(document.getElementById('editModal'), {
