...
<body>
    <div class="container-fluid">
        <div class="d-flex mt-4">
            <a href="{{route('AdminHome',['id'=>$id])}}"
                id="a" class="btn btn-primary mb-2"><i class="fas fa-arrow-left"></i></a>
        </div>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <button type="button" class="btn btn-primary btn-sm edit-btn" onclick="openEditModal(this)"
                                data-userid="{{ $usuario->id }}" data-username="{{ $usuario->name }}"
                                data-useremail="{{ $usuario->email }}" data-userrole="{{ $usuario->role }}" data-userphone="{{$usuario->phone}}">
                                Update
                            </button>
                            <!-- Botón para abrir el modal de eliminación -->
                            <button type="button" class="btn btn-danger btn-sm delete-btn"
                                onclick="openDeleteModal(this)" data-userid="{{ $usuario->id }}">
                                Delete
                            </button>
                        </td>
                    </tr>
 ...
 <script>
    function openEditModal(button) {
      ...
      // Verifica si ya existe una opción con el mismo valor
        var exists = false;
        for (var i = 0; i < selectElement.options.length; i++) { if (selectElement.options[i].value==userRole) { exists=true;
        break; } } // Si no existe, crea y agrega la nueva opción 
        if (!exists) { var
        option=document.createElement('option'); option.value=userRole; option.text=userRole; option.selected=true;
